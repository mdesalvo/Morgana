...
namespace Distiller2.Interfaces;
...
public interface IInterviewService
{
    ...
    /// <summary>
    /// Opens an agent of today's map at its Target for correction, setting the interview aside until
    /// the correction hands it back where it stood.
    /// </summary>
    /// <param name="entry">The map entry, at or before the one in hand.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns><c>false</c> when nothing can be reopened: the map is still being drawn, a correction
    /// is already open or the entry has no agent.</returns>
    // The correction runs in Correcting mode and nothing downstream is invalidated. It ends when the
    // agent is accepted again or when its chain reaches the step the interview stood on.
    Task<bool> ReopenAsync(int entry, CancellationToken cancellationToken = default);
