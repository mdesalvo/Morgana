...
- Starting a conversation under an ID already in use is now refused: **nobody can take over somebody else's conversation** by naming it at start
- **Cauldron (1)**
...
Morgana now speaks the **A2A protocol** instance to instance. Every agent is published at `/a2a/{intent}` with its well-known agent card. Agents can consult (and be consulted by) peers living on **remote Morgana instances**: `[ConsultsAgent("shipping", "acme")]` names an "acme" partner trusted under `Morgana:AgentToAgent:Partners` on which their agent "shipping" can be engaged and from which this Morgana may also be engaged.
...
- **Alembic has a brand new layout**: the workbench has been redesigned on Microsoft's **Fluent UI Blazor 5**

### 🐛 Fixed
- A conversation could not survive a Morgana restart: its first message afterwards failed without an answer and the agent the user was talking to was forgotten, so even a working reply would have treated the answer to that agent's question as a brand-new request
- A reply a webhook channel could not take at that moment was lost for good, leaving the turn to expire on the channel's own deadline
- When the model provider left a reply undated, the history dated it at the moment it was read, so a resumed conversation could list it out of place
- The classifier's confidence was written in the host's locale: this could make it read "0,95" instead of "0.95", so telemetry and any tool reading it back found no number
- Plain prose running over several lines was taken for Markdown, so a channel without Markdown support (e.g: Rune) paid an LLM rewrite for a reply that already fitted it
- **Cauldron (12)**
  ...
  - The replies missed across a reconnection were found by comparing the Cauldron host's clock with Morgana's: a reply could be missed or come back a second time
  - A reply cut short by a reconnection stayed on screen as a fragment beside the full reply recovered after it
  - A turn that went silent was given up on without checking whether Morgana had in fact answered it
...
Reaching Morgana from a browser has so far meant *being* Cauldron: the visitor had to land on the Blazor app. This release adds **Widget**, an embeddable launcher that puts a live Cauldron conversation into a page that already exists — whatever built it, JSP, PHP, ...
A single `<script src="https://your-cauldron-host/widget/morgana-widget.js" defer></script>`, with **no parameters at all**. The loader reads its own `src` to learn which Cauldron to open, so a snippet copied from a deployment points back at that deployment by construction.
Closed, it is a floating pill carrying Morgana's animated face and *Consult Morgana*; opened, it reveals a sandboxed `<iframe>` running the **real** Cauldron chat — streaming, rich cards, quick replies and dust gauge included — because it embeds the app rather than re-implementing a reduced copy of it.
Isolation runs in both directions and is browser-native rather than library-provided: a **closed shadow root** keeps the host page's CSS from deforming the launcher and the widget's CSS from leaking onto the host, while the iframe keeps the conversation inside Cauldron's own origin, unreadable from the embedding document.
Framing itself is **closed by default** — a site may host the widget only once its origin is listed in `Cauldron:Widget:AllowedEmbedOrigins`, emitted as a CSP `frame-ancestors` directive.
...
- **`Widget`** — a static-asset Razor Class Library (no C#, no Razor, no host of its own) published under `/widget/` by the very Cauldron instance it embeds, which is why the project reference points from Cauldron *to* the widget and not the other way round. It ships `morgana-widget.js` (origin discovery from its own `src`, closed shadow root, launcher, lazily mounted sandboxed iframe kept alive across close/open), `morgana-widget.css` (Cauldron's palette, `prefers-reduced-motion` aware, full-screen below 520px) and `morgana.html`, a sample host page carrying nothing but the script tag, with styling of its own so the isolation is visible at a glance. **Zero JavaScript dependencies** by design: a widget cannot know the stack of the page it lands in, so every library it brings is a conflict waiting to happen — Shadow DOM, `sandbox` and CSP cover what it actually needs
