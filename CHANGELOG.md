...
- **Wire contracts consolidated into `Morgana.Contracts`** — the DTOs exchanged between Morgana and its channels (channel handshake metadata/capabilities/coordinates, the outbound message envelope, quick replies, rich cards and the `CardComponent` hierarchy, the start/send request bodies and the webhook `StreamChunkRequest` chunk body) were previously duplicated in four places (Morgana.AI plus each of Cauldron/Rune/Grimoire). They now live once in a new **zero-dependency `Morgana.Contracts`** package: Morgana.AI references it and the three reference channels were rewired from their hand-maintained `Messages/Contracts/` copies to a **direct project reference** — a single source of truth, no more lockstep edits. Channel identity (the per-channel `ChannelMetadata` handshake) moved off the shared DTOs into channel-side factories. At `pack` time the project reference is emitted as a regular NuGet dependency, so `Morgana.Contracts` is published alongside `Morgana.AI`
