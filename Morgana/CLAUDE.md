...
**Parameter descriptions do not reach the model** — a known defect, not a design choice. `MorganaToolAdapter` assembles them and hands them to `AIFunctionFactoryOptions.AdditionalProperties`, which is metadata stored on `AITool.AdditionalProperties`, never part of the emitted JSON schema; MEAI takes parameter descriptions from the delegate's `[Description]` attributes, which no `MorganaTool` method carries. The schema goes out bare (`{"userId":{"type":"string"}}`), so every parameter description in `agents.json` — and every one an MCP server supplies, since MCP tools register through the same adapter — is dropped. What the model does receive: the tool name, the tool **description** (which is why `ToolDescriptionContextGuidance` works), and parameter names, types and required flags. Repairing it means emitting descriptions into the schema via `JsonSchemaCreateOptions`, which **adds** prompt text the model has never seen and therefore needs its own measured phase.
