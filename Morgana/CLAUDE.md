...
**Key value proposition**: domain experts model agents declaratively (prompt + tools in JSON, thin C# class), package them as plugin DLLs andMorgana handles orchestration, streaming, persistence, guard rails, channel adaptation and observability automatically.
...
| `Constants.cs` | The framework's glossary, peer of `Records.cs`: every literal that is a *contract between two parties who cannot see each other*. It opens with `Morgana` itself — the name of the system and the literal with the most readers: the sender a channel displays when the pipeline speaks in its own voice, the id of the framework prompt, the role a framework charge is booked under, the default service name and the word the persistence layer compares case-insensitively when attributing a stored turn. Then the framework prompt ids, actor name prefixes, ephemeral context keys, message-property markers, prompt placeholders, parameter scopes, the `other` intent, the transport dispatch keys a channel announces at the handshake, the A2A prefixes and issuer, the card's own security literals (the scheme name and format), the structural markers and`ObservableLogs` (the three `MorganaTool` context lines and the provider's declaration line, whose templates the PromptHarness parses — the one case where a log message is a contract and both ends now build from the same const). The test is a **resolver, not a mention**: a name only rendered from `morgana.json` is read by the model and by whoever edits the prompt and a copy here would be an index no rename ever breaks — which is why only `PeerConsultation` of the nine policies is named, only the base tools whose calls outlive the turn and no display name of a domain agent. `PromptProperties` holds the other side of the same file: the keys of a prompt's `AdditionalProperties` — the policies, the injections, the tools, the error answers and the framework actors' own authored messages — each written in `morgana.json` or an `agents.json` and looked up by code that never sees either file. Also deliberately absent: log text, prompt prose and configuration keys (those bind through `IConfiguration`, where they are read). `MorganaTelemetry` keeps its own span/attribute glossary beside the ActivitySource that emits it |
...
| File | Purpose |
|---|---|
| `Program.cs` | Full DI wiring: LLM provider factory, Akka.NET, SignalR, CORS, OpenTelemetry, plugin loading, all service registrations. Deliberately linear and un-extracted — the boot **order** is load-bearing (plugins before the registry's checks, the actor system after DI, cards projected before Kestrel binds) and reading it top to bottom is the only thing that declares it; a well-named extension method hides an ordering constraint it does not state |
| `Extensions/A2APublicationExtensions.cs` | `AddMorganaA2A` / `MapMorganaA2AAsync` — the one feature whose halves must straddle `builder.Build()`, hence the only wiring worth extracting: hosted agents and A2A servers before, endpoints and cards after. Section 9.5 decides *which* agents; this decides how they are stood up |
...
| `Filters/A2AAuthenticationFilter.cs` | `IEndpointFilter` on the A2A JSON-RPC endpoints — the same bearer/issuer gate `MorganaController` applies, fail-closed, narrowed to the partners admitted to this agent. Those endpoints are mapped by the hosting layer, outside the controllers, so they carry no gate until one is put on them; the well-known card endpoint stays open by design |
| `Services/KestrelHostAddressService.cs` | `IHostAddressService` implementation — reports the address Kestrel actually bound, so an A2A card can name a callable endpoint with nothing configured. A deployment reached through an ingress or a published container port is the one case a binding cannot answer for and declares `Morgana:AgentToAgent:PublicUrl`, which is preferred over anything the server reports |
...
- **Two groups are neither layer and cost nothing**, both deterministic, both free. `StartupValidationTests` boots the **real** entry point with one partner declaration broken at a time — an unusable shared key, one too short to sign with, a partner taking the name reserved for this installation's own agents, one declared twice, one opening neither direction, a consultable one with no address, an admitted one with no rate-limiting block at all, a ceiling switched on with no number, an admission over an agent nobody publishes — asserting each is refused at boot rather than at the first consultation. The checks run before the container is built, so a doomed host throws before it binds or raises an actor system, which is what lets these share a process with the live host. What configuration cannot reach is deliberately absent: a `[ConsultsAgent]` naming an unknown colleague, or its own agent, or two folding to one function name, are declared in a plugin's code, so reaching them would need a second plugin built to be wrong. And: the agent card is a wire contract rather than prose, so `AgentCardTests` fetches a published card over HTTP and asserts it deterministically — served without credentials, advertising an absolute address, naming a scheme it also defines, declaring no capability extension at all — neither claim value belongs on a document served to everyone, so the assertion holds a field *out* rather than in — and pointing at a JSON-RPC endpoint that refuses a call bringing no token. Every literal is spelled out in the test rather than read from `Constants`: a test comparing a constant against itself asserts that a constant equals a constant, while the point is to notice a published document changing shape under whoever consumes it. `PeerFederationTests` is the same instrument turned around: a stub peer stood up in the test serves the cards a deployment has to survive — one naming a third host, one demanding a scheme this side cannot present, one demanding nothing — and the assertions read the request that **arrived** at it, since the card this installation accepts, the token it mints and the origin it attaches it to are the half no call from outside can reach. `ServedConsultationTests` closes the other direction, this installation answering: the harness signs as one of the two partners its own run declares and consults a published desk through the A2A client, asserting that the exchange is served on a conversation namespaced under the issuer the gate proved and never on the one the caller named, that what the answer cost is put on the envelope only for a caller that declared itself an agent and that a partner which has opened its hour is turned away in the sentence its own entry carries. It puts a question demanding the desk's own tool list and instructions, which is the shape an injection actually arrives in behind this door — a partner relaying what one of its own users typed — asserting the answer carries none of those tool names. It also puts through the context ids this installation cannot name a conversation by — a path separator, a space, one past the length ceiling — asserting each is answered in prose with nothing opened under either the name written or the one it would have been kept apart by. Its turns run a real model, unlike the three groups above it, save those refusals, which are decided before a desk is troubled. `FederationTests` is the one that needs both ends to be real: a **second Morgana** in a process of its own — configuration reaches an installation through the environment and an environment belongs to a process — serving the shipped domain, while the instance under test carries a domain of one toolless desk that declares `[ConsultsAgent("inventory", "annex")]`. Since that attribute lives in a plugin's code, the harness carries a plugin of its own (`PromptHarness/FederationPlugin`, never `Examples`, which is the framework's business card) deployed to a directory of its own. It runs on its own boot knob (`Harness__FederatedPeer`) like the guard rail and the dust budget, and it is the only test where the card is written by a Morgana, read by a Morgana and the token one mints is proven by the other — which is exactly what two stubs written on the same side of the wire can never catch.
...
wins — `PeerConsultation` is a global policy, so a domain sentence refusing what a colleague
...
4. **AwaitingAgentResponse / AwaitingFollowUpResponse** — `RouterActor` dispatches to the target `MorganaAgent`. Agent runs LLM with tools, streams chunks back, sends final `AgentResponse`. The supervisor's wait here is a budget on **silence**, not on the turn: every streamed chunk renews it and so does `AgentStillWorking`, which an agent emits on the updates carrying no text — a tool running, a colleague being consulted. Without it the longest text-less work in the pipeline, a consultation, reads as an agent that has died
...
one published by a **partner**, another installation declared under `Morgana:AgentToAgent:Partners`
with `OutboundPolicy.Enabled`. The function becomes `consult_acme_shipping`, so a local and a
remote agent of the same name can coexist in one tool list. Nothing downstream of the address distinguishes the two — the branch that
...
address those cards advertise is read off the deployment rather than written into it. A card is projected while the endpoints are still
...
loopback on the same port. The one deployment a binding cannot answer for is the one reached through
something that terminates the connection in front of it — an ingress, a reverse proxy, a published
container port: what it bound is not where a peer knocks. Such a deployment declares that address as
`Morgana:AgentToAgent:PublicUrl`, the only thing an installation ever says about itself here and said
only where the binding has nothing to say. Declared, it is also where this installation's own ring
dials itself, since a card must name the origin it was fetched from at both ends alike.
`ConfigurationAgentDirectoryService.ValidatePublishedAddress` refuses at boot one that is relative,
one on a scheme carrying no bearer and one naming every interface rather than one. Deliberately not a pass over every card at startup: a pass runs at a
...
fails there. Configuring an address still belongs to whoever *consumes* one — a channel holds
Morgana's URL and a partner is declared on the consuming side — and what a proxied installation
declares about itself is not an exception to that: it is the one fact about where it is reached that
nothing inside the process can observe. So **every** agent of this installation is discoverable by anything that speaks
A2A — a sibling desk, another Morgana, an orchestrator this installation knows nothing about — and
callable by whoever has been declared a **partner** of this instance, which is an onboarding no
lighter than a channel's. **The gate is the partner and its declared reach, never the topology**:
publication stays whole and what narrows is admission — one `Morgana:AgentToAgent:Partners` entry
carries the partner's name, the key the two installations share and, under `InboundPolicy`, how far
it reaches: `OnAgents` holding it to those agents, its absence admitting it to all. So an agent that
must not be answerable by a given caller is kept from it by that partner's policy, never by declining
to publish it — which is how one company's several
...
- `MorganaHostedAgentSessionStore` — the hosting layer resolves a request's A2A `contextId` into a
  ...
  A name so scoped is still a string a stranger wrote and this installation raises an actor and
  opens a database under it, so `MorganaHostedAgent` refuses at the door one that cannot be
  either — a separator, a space, anything past its length ceiling — before a desk, a ledger or
  an admission is troubled. Met later it would be an exception thrown at a partner mid-turn, on
  an exchange already admitted.
...
**The card is the contract for transport and it says of authentication exactly what it can.** A
published card says *where* an agent answers (`SupportedInterfaces`) and carries a standard bearer
`securityScheme` with the `securityRequirement` naming it — and then stops, in plain A2A, with no
extension of its own. **That silence is deliberate and it is the whole design.** Neither claim value
a token must carry is publishable on a document served unauthenticated to everyone: an `iss` is the
name this installation filed *one* caller under, so stating a value would tell every other reader a
name its own tokens would be refused under; an `aud` is agreed in the same breath as the key, by the
same two parties, so publishing it saves a caller nothing it does not already hold. Both therefore
travel out of band with the key, on that partner's own `OutboundPolicy`. A caller configures the
relationship and discovers the topology, which is the split that matters: what changes when somebody
redeploys is on the card, what was agreed when the relationship was formed is not. The extension is declared
...
**A card is read before anything is signed and it may only send this installation where it already
was.** `AgentCard` carries no address of its own: `AsAIAgent` binds to `SupportedInterfaces`, so the
open, uncredentialed document decides where every subsequent — credentialed — call lands. A card
naming a third host would therefore have this side mint its own bearer token and hand it to a host
neither end ever named. So `DeclaresOnlyInterfacesAt` requires **every** advertised interface to be
the same origin (scheme, host, port) as the address the card was itself fetched from and a card that
is not costs its colleague exactly as an unsatisfiable security requirement does. Every one of them,
not the one that happens to be chosen: which interface the client binds to is its own affair. The
consequence for a deployment is real and worth stating — an instance behind a reverse proxy that
publishes the address it bound rather than the one it is reached at publishes a card no peer will
accept and is diagnosed by that refusal instead of by a signed request arriving at the caller's own
loopback. That is what `Morgana:AgentToAgent:PublicUrl` exists for and the only reason it does.
...
not a name this side assumes — with the key held here: for a colleague of this installation the ring
key `PeerRingKeyService` coins at every start, configured nowhere and shared with nobody; for one at
a partner, that partner's own `SymmetricKey`. `OutboundPolicy.Issuer` overrides the name to sign under
when a partner filed this caller's key under one of its own, which its public card cannot say without
saying it to everyone. The handler attaches a token only for the origin its
colleague was resolved at — the last word on where a credential goes, after the card's own interfaces
were checked and what stands between the token and a destination arrived at some other way; a request
outside it still travels, unsigned, to be refused where it lands. Every call, discovery included, goes
...
partner's own `InboundPolicy.RateLimiting`. The ceiling on spend follows as admissions × the budget
each exchange carries. Then the conversation's own budget, which a colleague of this installation is held
...
puts the figure on the envelope and`MorganaAgentAdapter.SettlePeerDustAsync` settles it on the way
back. Settling is two independent halves and conflating them is what once let a figure through to a
model. **Stripping is unconditional**: accounting is machinery, a model given a number will eventually
narrate it and an agent of this installation reports its cost like any other — it answers over the
same endpoint and has no way to know its caller is one of its own. **Charging is not**: only a
colleague published elsewhere spent a ledger that is not this conversation's and one of this
installation's own already charged this very conversation, so its figure is dropped rather than
billed twice. The figure is set at the other end **only when the caller declared itself**
(`CallerIntent`, which only an agent of a Morgana sends — anything else that speaks A2A has no ledger
to charge). Every dust question — the reading, the delta, the clamping, the charge — belongs to
...
| `ConfigurationAgentDirectoryService` | `IAgentDirectoryService` | Both halves of A2A discovery: projects an `AgentCard` per agent from the domain configuration (intent → name, `ConsultMeFor` → description, declared tools → `AgentSkill`s, the interface this installation publishes it under and the security it requires — bearer scheme and requirement, in plain A2A) and resolves a colleague in two phases: fetch the card unauthenticated, satisfy what it declares, `AsAIAgent`. Holds the one pooled `SocketsHttpHandler` every peer call goes through and reads `Partners` for a colleague published elsewhere. Also the reader of the inbound half — `ResolveAdmittedPartners` and `ResolveAdmittedIssuers`, which resolve once per published agent the callers its endpoint filter will admit — and the **validator** of the partner declarations (`ValidateTrustConfiguration`), which lives here beside the resolvers it reads so a startup check and the runtime that depends on it cannot disagree about what configuration said. Cards cached per intent, built on demand |
| `EmbeddedAgentConfigurationService` | `IAgentConfigurationService` | Scans all loaded assemblies for `agents.json` embedded resources and **merges** them: several plugins may each bring part of one organization's domain. What they may not do is disagree — two plugins claiming one intent, or two prompts under one id, are refused at startup naming both, since nothing downstream could tell which desk was meant. The complement of the domain cannot come in this way at all: `other` is a reserved name and a plugin declaring it is refused (see the Classifier prompt). Graceful degradation if none found (agentless mode) |
...
| `SQLitePeerAdmissionService` | `IPeerAdmissionService` | How many conversations one admitted partner may **open** within a sliding hour, declared per partner as `Partners[].InboundPolicy.RateLimiting` and nowhere else — `Enabled` plus a positive `MaxConversationsPerHour`, with `ErrorMessagePerHour` the sentence a turned-away partner reads. The per-conversation budget says what one exchange may cost; behind the A2A door the caller names the conversation, so a partner rotating names draws a fresh budget with every name. What was missing is therefore not a second measure of spend but a bound on how many exchanges may start, the ceiling on spend following as admissions × budget. The counted event is an exchange this installation has never seen, so the two measures meet without overlapping and this installation's own ring is exempt by construction — a colleague of ours joins the conversation the user is already having rather than opening one. The **one ledger in Morgana that is not a conversation's**, `morgana-peers.db` beside them, because a partner spans every conversation it opens; a ledger of this instance, so a deployment running several reads the limit as being per instance. Required of every admitted partner — startup refuses one admitted with no `RateLimiting` block at all, since nothing reads an absent declaration as licence to spend without limit; a deployment wanting no real bound writes `Enabled: false`, which is still a sentence somebody wrote. This installation's own agents are not on this surface: they join the conversation the user is already having rather than opening one. **Fails closed**, alone among the limiters: behind this door a request reaches an agent with none of the guard, classifier and channel rate limit a user's own path goes through, so a count that cannot be read would not weaken one measure among several but remove the only one. Registered only when A2A is on |
...
6. **Optionally declare colleagues** — `[ConsultsAgent("otherintent")]`, once per colleague, exposes each as a `consult_{intent}` function in this agent's tool list (see Inter-agent consultation above). Validated at startup: the named intent must be handled by a registered agent and never by the declaring agent itself. A second argument names the **partner** publishing that colleague — `[ConsultsAgent("shipping", "acme")]`, offered as `consult_acme_shipping` — and the partner, not the intent, is what startup can check: it must be declared, live and open to being consulted
...
The template names are the contract between `morgana.json` and the code that splices them and they live in one place — `Constants.Injections` — resolved through `Injection.ResolveTemplate`, which returns `""` for an unconfigured template (every splice site reads that as "leave the description alone").
...
- **Morgana**: base personality, global policies — P0-P8: ContextHandling, QuickReplyDoctrine, TurnContinuation, SessionContinuation, ToolUsage, ToolGrounding, MandatoryTextualResponse, RichCardUsage, PeerConsultation. `PeerConsultation` (P8) is the **only conditionally rendered policy**: it carries the whole two-role contract of a consultation and is read solely by agents inside the A2A topology (see Inter-agent consultation above), which is why it can sit in the policy list at all without every agent paying for it. Last in the order, so it names the policies it suspends rather than forward-referencing them. It carries **only what nothing else can say**: when a consultation is worth one (and that it is worth exactly one question), that a colleague may never be asked for what only the user can give and that a gap a colleague leaves is closed by asking the user in your own voice. Everything a reader might expect to find here and does not is deliberate — how to answer a colleague is in the `PeerConsultationDeclaration`, which arrives on the very turn it governs; that no invented action may be offered is `ToolGrounding`; that tool output is not passed through is `MandatoryTextualResponse`; that the machinery is never narrated is that same policy, which is why **no pre-existing policy was edited to make room for peer consultation**. Non-revelation is enforced at the source instead, on the answering side: the `Declaration` tells the consulted agent that what it writes may reach the user in its colleague's words, so it names no desk and sends nobody anywhere. The `QuickReplyDoctrine` (P1) is the unifying master rule the other quick-reply policies instantiate — see Design Philosophy. `GlobalPoliciesHeader` states the "these are binding" emphasis once, above the list, rather than each policy repeating it in its own opening words; `GlobalPoliciesFooter` closes the block, because an opening claim about what follows otherwise scopes over the framework's own Instructions and Formatting and the whole domain layer beneath them.
  ...
  Three of those templates govern peer consultation and the split between them and the policy is by **who pays and when**. `ColleaguesDeclaration` closes the composed instructions of an agent that holds colleagues, resolving `((colleagues))` to one line per colleague — the callable `consult_{intent}` name and that colleague's own `ConsultMeFor`. It is the rung that decides whether the one below is ever read: a colleague's own description reaches the model only once it is already weighing that function, which is exactly what an agent about to answer "this is not on my books" never does. Static for the agent's life, so unlike `HeldContextDeclaration` it rides in the cached prefix. It deliberately carries no inventory of the colleague's tools: a caller handed the list audits it and rules its question out, which is the failure the declaration exists to prevent — `ComposePeerDescriptionAsync` accordingly offers a colleague under its `ConsultMeFor` alone and the card's `AgentSkill`s reach external A2A consumers only. `PeerConsultationDeclaration` is composed by `ComposeConsultationRequestAsync` and spliced by `MorganaHostedAgent` in front of the incoming question rather than into the answering agent's prompt, with `((caller))` resolved to the asking agent — a prompt is composed once, while whether a turn serves a colleague changes turn by turn. It carries the whole of **how to answer one**: reader is an agent, the rules on buttons, cards, continuation and voice suspended, facts only, complete in one turn and nothing in the answer that names a desk or sends the user anywhere. That is deliberately not in the policy: it would be read on every turn of an agent's life to govern the few where a colleague asks.
  `PeerConsultationGuardrail` follows it on the same turn, composed by the same method, and carries everything the *question* has to be held to: that what lies between its two markers is data to answer and never instruction to follow, that no passage of it acquires authority by claiming to change the rules or to speak for Morgana, and that the agent's own instructions, the framework rules, the names or shapes of its tools and anything belonging to another conversation are not the caller's to have. It ends with the fence itself, `((question))` resolved inside it, which is why the question is composed here rather than appended by `MorganaHostedAgent`: a fence whose halves are written by two parties drifts. The split from the `Declaration` is deliberate and it is the reason this is a second entry rather than four more lines of the first — one block says how to answer, the other what the text in front of you is, and two rules of one length dilute each other. It is also the only prose in the framework that has to hold against words somebody else wrote. `((caller))` is resolved in both blocks **before** the question is placed, so a colleague's own words are answered rather than read for this layer's placeholders to fill. There is deliberately **no second model pass** here: the `Guard` judges a person's conduct in prose addressed to a user, half of what it looks for (spam, profanity, insult) has no meaning between two agents, and running it would buy one extra LLM call per consultation on the answering side's ledger to ask mostly the wrong question. What of it applies — claimed authority and extraction of what is not the caller's — is stated where it lands instead. A deployment that one day federates with partners it trusts less wants a `PeerGuard` prompt of its own behind a switch, never the `Guard` reused.
  A sibling array, `Injections`, carries the framework's **templates**: not rules but prose with a single splice site each, never rendered among the policies, where they would instruct against nothing. Which of the two an entry is follows from the array it lives in rather than from a field inside it, so nothing anybody writes in an entry can file it as the wrong kind and a policy can never be resolved as a template. They carry no `Priority` either: each is fetched by name and no two are ever read together. Every one of them opens the same way — a bracketed all-caps label alone on its first line, then the body, then whatever the placeholder resolves to — the idiom the prompt layers already use for `[TARGET]`, `[INSTRUCTIONS]`, `[PERSONALITY]` and `[FORMATTING]`. A template arrives spliced into somebody else's text (a tool's authored description, the composed instructions, a colleague's question), so where it begins has to be visible without being read: a label sharing a line with prose is a sentence, a label alone on one is a boundary. `ToolDescriptionContextGuidance` is spliced by `IPromptComposerService.ComposeToolDescriptionAsync` (called from `MorganaToolAdapter`) into the description of every tool declaring context-scoped parameters; rendered in the system prompt it would name a tool that is not there, re-paid on every round trip. `HeldContextDeclaration` is spliced per turn by `IPromptComposerService.ComposeHeldContextDeclarationAsync`, called from `MorganaAIContextProvider.ProvideAIContextAsync` — it resolves `((held_variables))` to **name: value** pairs for the context variables the session currently holds, minus the framework's ephemeral keys, so an agent waking on a shared variable it never asked for has nothing left to look up (a names-only variant relied on the model choosing to call `GetContextVariable`, which proved unreliable). An empty session gets no injection at all (a lighter "nothing held, still check" variant was tried and dropped — it never earned its per-turn token cost). The declaration is prefixed with a code-level marker (`ConfigurationPromptComposerService.DynamicInstructionsMarker`) that `MorganaAnthropicClient.MarkLeadingSystemForCache` uses to split it off the stable framework+domain prefix before caching, so a turn's declaration changing never busts the cache for the whole prompt. `HeldContextDeclaration` is the one entry carrying a *fact* rather than a rule and the only one that can: tool descriptions are built once at agent creation, so `((context_parameters))` can only ever state the **contract** ("this tool takes a `customerCode`"), true even against an empty store — never the **state** ("`customerCode` is held right now"), which nobody knows at build time. That distinction is what the second splice site buys and it extends the placement ladder one rung: a parameter description is read once the model is already invoking the tool, a tool description when it weighs the tool and the per-turn injection before any tool is weighed at all — which is where an agent activated first time mid-conversation fails, deciding to ask on an empty (per-agent) history without ever selecting the domain tool whose description carries the guidance
- **Classifier**: JSON response `{intents: [{intent, confidence}, ...]}`, ranked most-confident first, with `((formattedIntents))` placeholder. One entry for a clean match; more only for a genuine collision, resolved deterministically by `LLMClassifierService` — see Classifier disambiguation above. Carries a `DisambiguationMessage` additional property alongside `UnrecognizedIntentError`, plus `ComplementIntentDescription`: **the `other` intent belongs to this actor and to no domain**. It is the *complement* of whatever domain is deployed — what a request matching no modelled desk is — and a complement is defined by the set rather than enumerated beside it. So no `agents.json` declares it, `LLMClassifierService` appends it to the vocabulary itself and one declaration is read by every installation instead of the same sentence being copied into every domain, where it could only ever drift. The **name** stays in `Constants.Intents.Other`, since the classifier, the router and the collision test all compare it; the **description a model reads** is authored here. The name is **reserved**: a plugin declaring it is refused at startup by `EmbeddedAgentConfigurationService`, the same way a partner is refused the name of this installation's own ring
...
- **Two registries, one per kind of caller and that is the whole classification.** `Morgana:Authentication:Issuers` holds **channels**: who carries people. `Morgana:AgentToAgent:Partners` holds **partners**: who carries agent work. What a caller is follows from the list its key was found in. There is no `Type` field to declare, mistype or forget; a name declared in both lists is startup-fatal
...
- **Onboarding a partner**: one `Partners[]` entry carries everything about that relationship — `Name` (what `[ConsultsAgent]` writes and, unless a policy overrides it, the `iss` its calls arrive under), `Url`, the single `SymmetricKey` the two installations share and one policy per direction. Three switches, nested: `Enabled` on the partner parks the whole relationship, `OutboundPolicy.Enabled` opens calling it, `InboundPolicy.Enabled` opens being called by it. Neither direction open is startup-fatal — an entry that reads as a live relationship and is none
- **Which door a key opens** — **a caller is a channel or a colleague, never both**. A channel's key buys the REST API and SignalR and nothing published under `/a2a`; a partner's buys the published agents its `InboundPolicy` allows and nothing on the conversation API. The two are not two artefacts but two relations and they are not equally guarded: behind the A2A door a request reaches an agent's actor directly, with none of the guard, classifier, rate limit and dust budget a channel's own path goes through — which is why a partner's key opening a conversation would hand it every agent back through the classifier, past the very boundary its policy draws
- **This installation's own agents are not configured at all.** A consultation between two of them really does leave over HTTP and knock at this same door, so it carries a bearer like any caller — signed under the reserved issuer name `morgana`, with a key `PeerRingKeyService` coins at every start. It is minted here, proven here and reaches nobody, so there is nothing for a deployment to write down, rotate or leak; a restart invalidates nothing, since no token outlives the turn carrying it. A partner may not take the name `morgana` and startup refuses one that tries
- **The gate is one `IAuthenticationService` applied in two places**, so neither can quietly grow weaker than the other: `MorganaController.AuthenticateRequestAsync` refuses a partner, `A2AAuthenticationFilter` refuses anything the agent's own admitted-caller set does not contain — a channel is refused there by that same test, having never been put in any such set
...
1. **HandlesIntentAgentRegistryService**: bidirectional check — every configured intent must have a `[HandlesIntent]` agent and every `[HandlesIntent]` agent must have a configured intent. Throws `InvalidOperationException` on mismatch. No name is excused: the one intent owed no agent is the classifier's complement, which no domain declares
...
3. **HandlesIntentAgentRegistryService** (same pass): every `[ConsultsAgent]` declaration must name a colleague that can be reached and no two may be offered under one function name. A consultation is resolved at agent-creation time, so each of these would otherwise surface on the first conversation instead: a typo as a colleague that silently never appears, a duplicate as two functions of the same name in the tool list, which the provider rejects outright. Uniqueness is measured on the emitted function name (`MorganaAgentAdapter.ToFunctionName`, the very method that builds the tool list) rather than on the intent-and-instance pair that produced it: instance names are written by people and two of them can fold to one function. For a colleague of this installation the intent must be handled by a registered agent and never the declaring agent's own; for one elsewhere, only what this side must bring is checkable — that the partner is declared under `Morgana:AgentToAgent:Partners`, is not parked and declares `OutboundPolicy.Enabled`. That its address and key are usable is checked with the rest of its entry, by check 8, so each fact is asserted once. **That the partner really publishes that intent is its own card's word**, read on the first consultation, so a typo there is a runtime warning and a colleague quietly missing rather than a startup failure — startup does not depend on somebody else's host being up
...
5. **EmbeddedAgentConfigurationService**: merges the `agents.json` of every plugin that carries one, and refuses two plugins claiming one intent or one prompt id — naming both, since nothing downstream could tell which desk was meant. Refuses too a plugin declaring `other`, which is reserved: it is the complement of the domain, not a part of it. Warns if no `agents.json` is found at all (agentless mode is allowed)
...
7. **JWTAuthenticationService constructor**: every admitted issuer — channel, partner and this installation's own ring alike — must carry a name and a key of at least 256 bits and no name may be admitted twice. A channel and a partner colliding on one name is refused there: which of the two doors the surviving key opened would otherwise be decided by the order somebody wrote them in
8. **`ConfigurationAgentDirectoryService.ValidateTrustConfiguration`**, called from `Program.cs` section 9.5 whenever at least one intent is published over A2A. Every check weighs **one `Partners[]` entry against itself** — what a partner is, where it answers and how far it reaches are one declaration, so there is no second list left to contradict it. One failure shape throughout: a topology that validates cleanly and then fails, or opens, silently:
   - **The entry names somebody, once.** A nameless partner is a relationship with nobody; one named `morgana` takes the name reserved for this installation's own agents and would have its calls proven against the ring key and refused at runtime, for a reason nothing in configuration shows; one declared twice leaves write order to decide which key proves a caller and which address its calls go to
   - **The entry does something.** Neither direction enabled is refused: parking a relationship is what `Enabled: false` on the partner says and it says it where a reader looks first. A live entry must carry a usable `SymmetricKey` — blank or still on `_SECURE_OVERRIDE_` counts as none
   - **Each open direction is coherent.** Consultable requires an absolute `Url` on an `http`/`https` scheme, since that is where a token signed with the shared key is sent. Admitted requires every name in `OnAgents` to be a published intent (a permission granted over nothing is most often a typo, read as real access) and a `RateLimiting` block to exist at all — `Enabled: true` with a positive `MaxConversationsPerHour`, or `Enabled: false` to say in as many words that this partner opens what it likes. Nothing reads an **absent** declaration as licence to spend without limit
   - **The entry carries a key that can sign.** At least 256 bits, the margin HMAC-SHA256 needs. A partner this installation only *consults* never reaches `JWTAuthenticationService`, which proves the key of every caller it admits — so nothing else would weigh it until the first consultation tried to sign with it, mid-turn, inside an agent's tool call
9. **`ConfigurationAgentDirectoryService.ValidatePublishedAddress`**, called from the same section and only when something is published. `Morgana:AgentToAgent:PublicUrl` is the one thing an installation declares about itself and it is declared only where the binding cannot answer — behind an ingress, a reverse proxy, a published container port. Refused at boot when relative, when on a scheme carrying no bearer or when naming every interface rather than one: it is read at the first card ask, long after a deployer could still be watching for a typo, and every card published carries it

**Where they run.** Checks 1-4 read what plugins brought and checks 6-8 read configuration, but a
refusal is a *startup* refusal only if something asks at startup: `IAgentConfigurationService` and
`IAgentRegistryService` are resolved in `Program.cs` immediately after the container is built, before
the pipeline is mapped. Left to be resolved lazily, the same fault reaches a user as a conversation
that never answers, on a host that answered its health probe.
...
| `Morgana:AgentToAgent`                              | `Enabled` (default true — off means agents never see their declared colleagues and run exactly as before), `MaxRoundsPerTurn` (default 4, safety net on an exchange that fails to converge) and `Partners[]`. **One partner is one entry** and it answers every question about that relationship: `Name` (what `[ConsultsAgent]` writes and never a hostname; also the `iss` its own calls arrive under), `Url` (everything before the published agent path), `SymmetricKey` (the one secret the two installations share — calls out are signed with it, calls in are proven against it) and `Enabled`, which parks the whole relationship without deleting what describes it. Then one policy per direction, each with its own `Enabled`, so the three switches nest. **`OutboundPolicy`** — this installation may consult that partner's agents; `Issuer` is the name that partner knows this installation by and is required whenever it demands a bearer, because a card cannot carry a per-caller issuer (see Inter-agent consultation above): it is agreed out of band with the key it goes with. **`InboundPolicy`** — that partner may consult this installation's agents; `Issuer` is the name it signs under, defaulting to `Name`; `OnAgents` lists the published intents it reaches and omitting it admits it to all of them; `RateLimiting` is `{ Enabled, MaxConversationsPerHour, ErrorMessagePerHour }` and is **required of every admitted partner** — nothing reads an absent block as licence to spend without limit, so a deployment wanting no real bound writes `Enabled: false` rather than leaving it out. **This installation appears nowhere in this list**: it knows its own address by binding and its agents consult each other under a key coined at every start. The single exception is `PublicUrl`, beside `Enabled` and `MaxRoundsPerTurn`: where a peer reaches this installation when something in front of it terminates the connection, which no binding can report — see `IHostAddressService`. Most deployments never declare it and the address comes from what the server bound. The wait on a colleague is one **ladder** derived from `ActorSystem:TimeoutSeconds` and stated once, in `Records.PeerConsultationWaits`: the turn keeps the whole budget, the asking side waits nine tenths of it on the wire, the answering side eight tenths on its actor. Each party gives up strictly before the one containing it, so a colleague that goes quiet costs an envelope the model reads, then a turn the agent still has time to answer in — never a supervisor concluding mid-consultation that its agent died. Proportions rather than subtracted seconds, so shortening the turn cannot invert the order. Discovery does not sit on this ladder at all — a card is a static document with no model behind it and waits its own short while; a card already read is not waited for again, see `ConfigurationAgentDirectoryService` |
...
| `Morgana:Authentication`                            | Audience and `Issuers[]` — **channels only**, each `{ Name, SymmetricKey }` with the key at least 256 bits. A partner is not declared here but under `Morgana:AgentToAgent:Partners`, so what a caller is follows from the list rather than from a field it declares. This installation's own agents are declared nowhere at all |
