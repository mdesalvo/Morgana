...
with `OutboundPolicy.Enabled`, and the function becomes `consult_acme_shipping` so a local and a
...
consequence for a deployment is real and worth stating — an instance behind a reverse proxy whose
`IHostAddressService` reports the address it bound rather than the one it is reached at publishes a
card no peer will accept and is diagnosed by that refusal instead of by a signed request arriving at
the caller's own loopback.
...
8. **`ConfigurationAgentDirectoryService.ValidateTrustConfiguration`**, called from `Program.cs` section 9.5 whenever at least one intent is published over A2A. Every check weighs **one `Partners[]` entry against itself** — what a partner is, where it answers and how far it reaches are one declaration, so there is no second list left to contradict it. One failure shape throughout: a topology that validates cleanly and then fails, or opens, silently:
   ...
   - **What is no longer checked because it can no longer be written**: the local ring needs no issuer, no key and no admission, so the five checks that guarded those have nothing left to guard
