...
goes through and not a lower bar. **The gate is the issuer and its declared reach, never the
topology**: publication stays whole and what narrows is admission — an issuer is declared
...
card cannot say without saying it to everyone. The handler attaches a token only for the origin its
...
- **Identity and reach are separate questions.** `Issuers` answers *who may knock* — one global registry, inbound only, read by both gates and a caller absent from it is refused at either. `Morgana:AgentToAgent:InboundSystems` answers *how far a knocker reaches* and admits nobody by itself. `OutboundSystems` answers neither: calling a system requires no `Issuers` entry for it, since that key and issuer name are the callee's, declared on the outbound entry
- **Onboarding a new channel**: any channel beyond Cauldron must be declared as an `Issuers[]` entry on the destination Morgana instance — its `Name` must equal the channel's `iss` claim, its `SymmetricKey` must match the secret the channel uses to sign tokens and its `Type` must be `channel`. A channel whose issuer name is not declared, or whose key does not match, is rejected at the very first request (fail-closed)
- **What an issuer buys and which door**: `Type` is mandatory on every entry and decides that door — **a caller is a channel or a colleague, never both**. `channel` buys the REST API and SignalR and nothing published under `/a2a`; `system` buys the published agents its `InboundSystems` entry allows and nothing on the conversation API. The two are not two artefacts but two relations and they are not equally guarded: behind the A2A door a request reaches an agent's actor directly, with none of the guard, classifier, rate limit and dust budget a channel's own path goes through — which is why a system's key opening a conversation would hand it every agent back through the classifier, past the very boundary its scope draws
- **The gate is one `IAuthenticationService` applied in two places**, so neither can quietly grow weaker than the other: `MorganaController.AuthenticateRequestAsync` refuses a `system`, `A2AAuthenticationFilter` refuses a `channel` and then anything the agent's own admitted-issuer set does not contain
...
3. **HandlesIntentAgentRegistryService** (same pass): every `[ConsultsAgent]` declaration must name a colleague that can be reached and no two may be offered under one function name. A consultation is resolved at agent-creation time, so each of these would otherwise surface on the first conversation instead: a typo as a colleague that silently never appears, a duplicate as two functions of the same name in the tool list, which the provider rejects outright. Uniqueness is measured on the emitted function name (`MorganaAgentAdapter.ToFunctionName`, the very method that builds the tool list) rather than on the intent-and-instance pair that produced it: instance names are written by people and two of them can fold to one function. For a colleague of this installation the intent must be handled by a registered agent and never the declaring agent's own; for one elsewhere, only what this side must bring is checkable — that the system is declared under `Morgana:AgentToAgent:OutboundSystems`, carries an absolute `Url` on an `http`/`https` scheme (it is where a token minted with that key is sent) and a usable `SymmetricKey`. **That the instance really publishes that intent is its own card's word**, read on the first consultation, so a typo there is a runtime warning and a colleague quietly missing rather than a startup failure — startup does not depend on somebody else's host being up
...
7. **JWTAuthenticationService constructor**: every declared issuer must carry a `Type` — `channel` or `system`. Checked beside the `Name` and key checks it belongs with and therefore whenever the service is built, A2A on or off. The option is nullable precisely so absence is distinguishable: defaulting it would classify a caller by enum ordering and the classification decides whether a key reaches an agent's actor bypassing the whole conversation pipeline
...
| `Morgana:Authentication`                            | Audience, Issuers[] (per-issuer Name + SymmetricKey min 256-bit + **mandatory `Type`**, `channel` or `system`). Beyond the channels, one entry named `morgana` typed `system` is required wherever any agent declares a colleague **of this installation**: it is the issuer the instance signs its own agent-to-agent traffic with and is admitted back in under, so a missing or still-placeholder key, a wrong `Type` or a missing `InboundSystems` entry are all startup-fatal (see Startup Validation 8). An installation whose agents consult only colleagues elsewhere, or none, needs no such entry                                                                                                                                                              |
