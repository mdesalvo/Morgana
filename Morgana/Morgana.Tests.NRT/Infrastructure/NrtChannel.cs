...
public sealed class NrtChannel : IAsyncDisposable
{
    ...
    /// <summary>HMAC-SHA256 credentials matching the <c>nrt</c> issuer key the host was started with.</summary>
    ...
    /// <param name="issuerKey">Symmetric key the host accepts for the <c>nrt</c> issuer.</param>
    /// <param name="audience">Audience claim Morgana validates.</param>
    public NrtChannel(string morganaBaseAddress, string issuerKey, string audience)
    {
    ...
    public async Task StartAsync()
    {
        ...
        receiver.MapPost("/nrt-hook", async (HttpContext context) =>
        {
        ...
        receiver.MapPost("/nrt-hook/chunk", async (HttpContext context) =>
        {
        ...
        callbackUrl = $"{address.TrimEnd('/')}/nrt-hook";
    }
    ...
    private string GenerateToken()
    {
        SecurityTokenDescriptor descriptor = new SecurityTokenDescriptor
        {
            ...
            [
                new Claim(JwtRegisteredClaimNames.Sub, "nrt-harness"),
                new Claim(JwtRegisteredClaimNames.Name, "NRT")
            ]),
