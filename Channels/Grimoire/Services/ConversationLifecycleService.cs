...
/// <summary>
/// Drives the one conversation a Grimoire process lives for: opens it with Morgana, waits for the
...
{
    /// <summary>Fallback for <c>Grimoire:StartupTimeoutSeconds</c> when absent or non-positive.</summary>
        ...
        IConfiguration configuration)
    {
        ...
        int startupTimeoutSeconds = configuration.GetValue<int?>("Grimoire:StartupTimeoutSeconds") ?? DefaultStartupTimeoutSeconds;
    ...
    private Task RouteWebhookToUi()
    {
        ...
        webhookReceiverService.OnChunk = chunk => consoleUiService.EnqueueChunk(chunk.ChunkText);
